class GlitchFX {
  ...
  GlitchFX(PApplet mainApplet_, PImage src_) {
    src = src_.copy();
    src_original = src_.copy();
    w = src.width;
    h = src.height;
    glitchFilter = new Filters();
    cvFilter = new FiltersCV(mainApplet_);
  }
  ...
  void mosh() {
    ...
    for (int i = 0; i < N; i++) {
      ...
      case 7:
        ...
        break;
      case 8:
        overwriteSource(glitchFilter.invertSaturation(next_step));
        print(" INVERT SATURATION");
        break;
      case 9:
        //we're only gonna do this effect if it's the first, up to the fourth~ as in N = 0 to 3
        if (N <= 3) {
          overwriteSource(cvFilter.edgeDetector(next_step));
          print(" EDGE DETECTOR");
        } else {
          print(" FX BYPASSED");
        }
